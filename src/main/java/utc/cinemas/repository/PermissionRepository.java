...
public interface PermissionRepository extends JpaRepository<Permission, Long> {
    ...
    @Query("SELECT p FROM Permission p WHERE (p.permissionName LIKE :search OR p.description LIKE :search OR p.permissionCode LIKE :search) AND (:module = '-1' OR p.module = :module) AND p.display = 1")
